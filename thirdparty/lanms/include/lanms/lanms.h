...
// From https://github.com/argman/EAST/tree/master/lanms
// locality-aware NMS
namespace lanms {
    ...
    inline bool should_merge(const Polygon &a, const Polygon &b, float iou_threshold) {
	 ...
	 */
    inline std::vector<Polygon> standard_nms(std::vector<Polygon> &polys, float iou_threshold) {
		size_t n = polys.size();
		if (n == 0)
			return {};
