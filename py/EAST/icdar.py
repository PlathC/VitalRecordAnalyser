...
tf.app.flags.DEFINE_string('training_data_path', './output/',
                           'training dataset to use')
tf.app.flags.DEFINE_string('gt_data_path', '', 'groundtruth related to the dataset')
tf.app.flags.DEFINE_integer('max_image_large_side', 6000,
...
def get_xml_files():
    files = []
    files.extend(glob.glob((os.path.join(FLAGS.gt_data_path, '*.xml'))))
    return files
...
def load_annotations_xml(file):
    text_polys = []
    text_tags = []
    root = ET.parse('./botany/Botany_Train.xml').getroot()
    words = root.findall("spot/[@image='{}']".format(file))
    ...
    for word in words:
        ...
        text_polys.append([[x, y], [x+w, y], [x+w, y+h], [x, y+h]])
        text_tags.append(False)

    return np.array(text_polys, dtype=np.float32), np.array(text_tags, dtype=np.bool)


def load_annotations_json(data, img_name):
    text_polys = []
    text_tags = []

    if img_name in data:
        for box in data[img_name]:
            x = int(box[0])
            y = int(box[1])
            w = int(box[2])
            h = int(box[3])
...
def load_json(file):
    ...
    with open(file, 'r') as file:
        ...
        for key in data:
            output[os.path.normcase(key)] = []
            for box in data[key]:
                output[os.path.normcase(key)].append([box[0], box[1], box[2], box[3]])
...
def generator(input_size=512, batch_size=32,
    ...
    training_boxes = load_json(os.path.join(FLAGS.training_data_path, 'output.json'))
    while True:
        ...
        for i in index:
            try:
                ...
                # print im_fn
                h, w, _ = im.shape
                # txt_fn = im_fn.replace(os.path.basename(im_fn).split('.')[1], 'xml')
                # if not os.path.exists(txt_fn):
                #     print('text file {} does not exists'.format(txt_fn))
                #     continue

                text_polys, text_tags = load_annotations_json(training_boxes, os.path.normcase(im_fn))
